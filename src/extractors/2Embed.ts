...
export async function getTwoEmbed(tmdb_id: string | number, s: string | number, e: string | number) {
    subtitles = [];
    sources = [];
    ...
    try {
        ...
        return { sources: sources.length > 0 ? { sources } : [createErrorObject("No sources found after processing")] };

    } catch (error) {
        return { sources: [createErrorObject(`Unexpected error: ${(error as any).message}`)] };
    }
...
async function parseSubs(scriptstring: string) {
    ...
    try {
        ...
        const subtitleTracks = (setupConfig.tracks || []).filter((track: { kind: string; file: string; label: string }) =>
        ...
        subtitleTracks.forEach((newSub: { url: string; lang: string }) => {
            if (!subtitles.some((existingSub: {url: string, lang: string}) => existingSub.url === newSub.url && existingSub.lang === newSub.lang)) {
