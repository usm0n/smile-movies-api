...
function createErrorObject(errorMessage: string) {
  return {
    provider: "EmbedSU",
    ERROR: [
      {
...
export async function getEmbedSu(tmdb_id: string, s: string, e: string) {
  ...
  try {
    ...
    if (!htmlSearch.ok) {
      return {
        sources: [
          createErrorObject(
            `Failed to fetch initial page: HTTP ${htmlSearch.status}`
          ),
        ],
      };
    }
    ...
    if (!hashEncode) {
      return {
        sources: [createErrorObject("No encoded hash found in initial page")],
      };
    }
    ...
    try {
      ...
      return {
        sources: [
          createErrorObject(`Failed to decode initial hash: ${e.message}`),
        ],
      };
    }
    ...
    if (!mEncrypt) {
      return {
        sources: [createErrorObject("No encrypted hash found in decoded data")],
      };
    }

    let firstDecode;
    try {
      firstDecode = (await stringAtob(mEncrypt))
        .split(".")
        .map((item) => item.split("").reverse().join(""));
    } catch (e: any) {
      return {
        sources: [
          createErrorObject(`Failed to decode first layer: ${e.message}`),
        ],
      };
    }
    ...
    let secondDecode;
    try {
      secondDecode = JSON.parse(
        await stringAtob(firstDecode.join("").split("").reverse().join(""))
      );
    } catch (e: any) {
      return {
        sources: [
          createErrorObject(`Failed to decode second layer: ${e.message}`),
        ],
      };
    }
    ...
    if (!secondDecode || secondDecode.length === 0) {
      return {
        sources: [createErrorObject("No valid sources found after decoding")],
      };
    }
    ...
    for (const item of secondDecode) {
      try {
        ...
        const dataDirect: any = await requestGet(urlDirect, {
          Referer: DOMAIN,
        ...
        const tracks = (dataDirect.subtitles || [])
          .map((sub: { file: string; label: string }) => ({
            url: sub.file,
        ...
        if (!requestDirectSize.ok) {
          console.warn(
            `Failed to fetch source ${dataDirect.source}: HTTP ${requestDirectSize.status}`
          );
          continue;
        }
        ...
        const directQuality = patternSize
          .map((patternItem) => {
            try {
              ...
              return {
                file: dURL,
                type: "hls",
                quality: `${sizeQuality}p`,
                lang: "en",
              };
            } catch (e) {
              console.warn(
                `Failed to process quality for pattern: ${patternItem}`,
                e
              );
              return null;
            }
        ...
        if (!directQuality.length) {
          console.warn(
            `No valid qualities found for source ${dataDirect.source}`
          );
          continue;
        }
    ...
    if (sources.length === 0) {
      return {
        sources: [
          createErrorObject(
            "No valid sources found after processing all items"
          ),
        ],
      };
    }
...
function getSizeQuality(url: string) {
  try {
...
async function stringAtob(input: string) {
  const chars =
...
async function requestGet(url: string, headers = {}) {
  try {
