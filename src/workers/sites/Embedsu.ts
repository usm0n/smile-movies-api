  ...
  try {
    ...
    for (const item of secondDecode) {
      try {
        ...
        if (!dataDirect || !(dataDirect as any).source) {
          console.warn(`No source found for hash ${item.hash}`);
          continue;
        }

        const tracks = ((dataDirect as any).subtitles || []).map((sub: any) => ({
          url: sub.file,
          lang: sub.label ? sub.label.split('-')[0].trim() : 'en'
        })).filter((track: any) => track.url);

        const requestDirectSize = await fetch((dataDirect as any).source, { headers, method: "GET" });
        if (!requestDirectSize.ok) {
          console.warn(`Failed to fetch source ${(dataDirect as any).source}: HTTP ${requestDirectSize.status}`);
          continue;
        }
        ...
        if (!directQuality.length) {
          console.warn(`No valid qualities found for source ${(dataDirect as any).source}`);
          continue;
        }
