...
import JsUnpacker from "../utils/jsunpack";
...
function parseQuality(qualityString: string) {
...
export async function getTwoEmbed(tmdb_id: string, s?: string, e?: string) {
  subtitles = [];
  sources = [];

  const tmdbId = tmdb_id;
  const initialReferer =
    s && e
      ? `${DOMAIN}/embedtv/${tmdbId}&s=${s}&e=${e}`
      : `${DOMAIN}/embed/${tmdbId}`;
  ...
  try {
    ...
    let match = data.match(/swish\?id=([\w\d]+)/);
    ...
    if (extractedValue) {
      if (isSwishId) {
        ...
        if (!streamUrl) {
          return {
            sources: createErrorObject(
              `Could not resolve stream URL for swish ID: ${extractedValue}`
            ),
          };
        }
        ...
        if (parsedSources.length > 0) {
          ...
          return {
            sources: createErrorObject(
              "No valid sources found in M3U8 for swish ID"
            ),
          };
        }
      } else {
        ...
        if (bestPartialUrl) {
          const idMatch = (bestPartialUrl as any).match(/\?id=([\w\d]+)/);
          if (idMatch && idMatch[1]) {
            ...
            if (parsedSources.length > 0) {
              ...
              return {
                sources: createErrorObject(
                  "No valid sources found in M3U8 for player4u ID"
                ),
              };
            }
          } else {
            return {
              sources: createErrorObject(
                "Could not extract player4u ID from best quality URL"
              ),
            };
          }
        } else {
...
async function resolve(url: string, referer: string) {
  try {
    const response = await fetch(url, {
      headers: {
        Referer: referer,
...
async function parseM3U8(m3u8Url: string, referer: string) {
  const parsedSources = [];
  try {
    ...
    if (!response.ok) {
      console.error(
        `Failed to fetch M3U8 ${m3u8Url}: Status ${response.status}`
      );
      return [];
    }
...
async function parseSubs(scriptstring: string) {
  ...
  try {
    ...
    const subtitleTracks = (setupConfig.tracks || [])
      .filter(
        (track: { kind: string; file: string; label: string }) =>
          track &&
          (track.kind === "captions" || track.kind === "subtitles") &&
          track.file
      )
      .map((track: { kind: string; file: string; label: string }) => ({
        url: track.file,
        lang: track.label || track.kind,
      }));

    subtitleTracks.forEach((newSub: { url: string; lang: string }) => {
      if (
        !subtitles.some(
          (existingSub) =>
            existingSub.url === newSub.url && existingSub.lang === newSub.lang
        )
      ) {
